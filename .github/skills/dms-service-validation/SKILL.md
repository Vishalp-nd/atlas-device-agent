...
2. **Check device type** — skip all DMS recording tests if device is NOT `bagheera3` or `octo`; service-status tests (TC_520, TC_524, TC_534, TC_541, TC_2830, TC_2831, TC_4159) run on any device type; haptic tests (TC_3801, TC_4157, TC_4161, TC_4162) run on both `bagheera3` and `octo` — `haptic_feedback.cpp` has zero device_type branching, the feature is config/enablement-only ([GPIO], [haptic_motor_health_monitoring] sections + systemd_override_octo.sh unit inclusion on Octo). TC_4162 does branch on device_type for one log string only: `"USB hub detected"` on bagheera3 vs `"USB hub device 1 connected"` on octo
